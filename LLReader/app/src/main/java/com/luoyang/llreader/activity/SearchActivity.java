...
import com.luoyang.llreader.presenter.impl.SearchPresenterImpl;
...
import com.luoyang.llreader.view.adapter.SearchHistoryAdapter;
import com.luoyang.llreader.widget.flowlayout.TagFlowLayout;
import com.luoyang.llreader.widget.refreshview.OnLoadMoreListener;
...
public class SearchActivity extends MBaseActivity<ISearchPresenter> implements ISearchView {
    ...
    private void toSearch() {
        if (edtContent.getText().toString().trim().length() > 0) {
            ...
            closeKeyBoard();
            Log.e(Common.TAG,"toSearch closeKeyBoard();");
            //执行搜索请求
    ...
    private void bindKeyBoardEvent() {
        llSearchHistory.getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
            ...
            public void onGlobalLayout() {
                ...
                Log.e(Common.TAG,"toSearch height:" + height +" ,r.bottom "+r.bottom+",diff:"+diff);
                //if (diff != 0 && Math.abs(diff) != StatusBarUtils.getNavi_height()) {
                if (Math.abs(diff) > (height/4)&& Math.abs(diff) != StatusBarUtils.getNavi_height()) {
